...
namespace eZdravaIshrana_API.Controllers
{
    public class OcjeneController : ApiController
    {
        ...
        [HttpGet]
        [Route("api/Ocjene/CheckIF/{kupacID}/{proizvodID}")]
        public Ocjene CheckIF(int kupacID, int proizvodID)
        {
            return db.Ocjene.Where(x => x.KupacID == kupacID && x.ProizvodID == proizvodID).FirstOrDefault();
        }

        [HttpGet]
        [Route("api/Ocjene/GetProsjek/{proizvodID}")]
        public List<Ocjene> GetProsjek(int proizvodID)
        {
            return db.Ocjene.Where(x => x.ProizvodID == proizvodID).ToList();
        }
        // GET: api/Ocjene/5
